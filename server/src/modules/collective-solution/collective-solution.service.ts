...
import { CollectiveSolution, FeCollectiveSolution } from "~/helpers/types";
...
export class CollectiveSolutionService {
  ...
  public async startNewSolution(
    user: IPartialUser,
  ): Promise<FeCollectiveSolution> {
    const alreadyScrambledSolution = await this.collectiveSolutionModel.findOne(
      { state: 10 },
    ).exec();
    if (alreadyScrambledSolution) {
      throw new BadRequestException({
        ...
        data: {
          collectiveSolution: this.mapCollectiveSolution(
            alreadyScrambledSolution,
          ),
        },
      });
    }
    ...
    this.logger.logAndSave(
      "Generating new Collective Cubing scramble.",
      LogType.StartNewSolution,
    );
    ...
    const newCollectiveSolution: CollectiveSolution = {
      ...
      scramble,
      solution: "",
      lastUserWhoInteracted: new mongo.ObjectId(user._id as string) as any,
      usersWhoMadeMoves: [],
      state: 10,
    };

    const newSolution = await this.collectiveSolutionModel.create(
      newCollectiveSolution,
    );

    const oldSolution = await this.collectiveSolutionModel.findOne({
      state: 20,
    }).exec();
  ...
  public async makeMove(
    ...
    if (!currentSolution) {
      throw new BadRequestException(
        "A solve hasn't been started yet. Please generate a scramble first.",
      );
    }
    ...
    if (currentSolution.solution !== makeMoveDto.lastSeenSolution) {
      throw new BadRequestException({
        ...
        data: {
          collectiveSolution: this.mapCollectiveSolution(currentSolution),
        },
      });
    }

    currentSolution.solution = `${currentSolution.solution} ${makeMoveDto.move}`
      .trim();
    currentSolution.lastUserWhoInteracted = new mongo.ObjectId(
      user._id as string,
    ) as any;
    if (
  ...
  async getCurrentSolution(): Promise<CollectiveSolutionDocument> {
    return await this.collectiveSolutionModel.findOne({ state: { $lt: 30 } })
      .exec();
  }

  private mapCollectiveSolution(
    collectiveSolution: CollectiveSolutionDocument,
    ...
    const mappedCollectiveSolution: FeCollectiveSolution = {
      ...
      lastUserWhoInteractedId: collectiveSolution.lastUserWhoInteracted
        .toString(),
  ...
  async getIsSolved(
    ...
    const scramble = new Alg(
      `${collectiveSolution.scramble} z2 ${collectiveSolution.solution}`,
    );
